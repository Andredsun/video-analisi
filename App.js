...
const App = () => {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      setSelectedVideo({uri: result.assets[0].uri, localFileName: result.assets[0].assetId + (Math.random() + 1).toString(36).substring(7), creationDate: ''});
    }
  };
...
{/*               <Button title='TEST' onPress={() => {
               ...
               setInterval(() => {
                ...
                if(currentTime.current > 10) currentTime.current = 0
                currentTime.current += 0.1
                videoPlayerRef.current?.seek(currentTime.current);
